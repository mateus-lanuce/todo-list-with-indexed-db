...
function showTodos() {
  ...
  objectStore.openCursor().addEventListener('success', e => {
    ...
    if(pointer) {
      ...
      listItem.classList.add('list-group-item', 'border-info', 'border-2')
      ...
      if(!todos.firstChild) {
        const listItem = document.createElement('li');
        listItem.classList.add('list-group-item', 'border-danger', 'border-2')
        listItem.textContent = 'Sem Todo.'
        todos.appendChild(listItem);
      }
...
function deleteItem(e) {
  ...
  transaction.addEventListener('complete', () => {
    ...
    alert(`Todo de id ${todoId} deletada`)
    console.log(`Todo:${todoId} deletado.`);
    if(!todos.firstChild) {
      const listItem = document.createElement('li');
      listItem.classList.add('list-group-item', 'border-danger', 'border-2')
      listItem.textContent = 'Sem todos.';
      todos.appendChild(listItem);
    }
  });
  transaction.addEventListener('error', () => console.log('Transaction error'));
